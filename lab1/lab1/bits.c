 ...
 */
int thirdBits(void) {
  // initialize the starting 8 bits
  int number = 0x49; 
  int number1, number2, number3;
  // shift by 8 bits to the left and add next 8 bits untill 32-bit number is formed
  number1 = (number << 8) + 0x24; // 16 bits
  number2 = (number1 << 8) + 0x92; // 24 bits
  number3 = (number2 << 8) + 0x49; // 32 bits
  return number3;
}
/* 
...
int sign(int x) {
  int msb = x >> 31;
//double negation shows if x is 0 or 1  
  return msb | (!!x);
}
/* 
...
int getByte(int x, int n) {
  // shift then mask approach
  // shift n left by 3 means multiplying n by 8. 
  int mask = 0xff;
  return (x >> (n << 3)) & mask;
}
// Rating: 3
...
int logicalShift(int x, int n) {
  // regular (arithmetic) right shift by 4 that propagates the MSB 
  int arShift = x >> n;
  // created bit mask replaces 1's with 0's in the final answer 
  int bitMask = ~(((0x1 << 31) >> n) << 1);
  
  return arShift & bitMask;
}
/* 
...
int addOK(int x, int y) {
  // if the sum of two positive numbers results in a negative number -> overflow
  // if the sum of two negative numbers results in a positive number -> overflow
  // solution: compare msb values of x and y against the sum of the two
  int msbX = x >> 31;
  int msbY = y >> 31;
  int sumXY = x + y;
  int msbSum = sumXY >> 31;
  // logical NOT returns 0 if the result is true, e.g. no overflow detected
  return !(~(msbX ^ msbY) & (msbX ^ msbSum));
}
...
int invert(int x, int p, int n) {
  // create a mask with every bit set to 0 besides the region specified by p and n
  // XOR operation allows to invert the original bits only in the needed region
  int mask = (~0<<p)^(~0<<(p+n));
  return x^mask;
}
// Rating: 4
 ...
 */
int bang(int x) {
  // if msb is 1 the number is not 0; 
  // the number has to be negative after substractng it from 0 -> x is not zero
  return ~((x | (~x + 1)) >> 31) & 1;
}
