 ...
 */
  ...
  // You will want to replace this return statement...
   ...
   // get a big enough block for the requested size; return a pointer to a beginning of that block
  ...
  // get a block size of the allocated block
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  // set the preceding block tag; update the header
  precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
  // remove that block from the free list
  removeFreeBlock(ptrFreeBlock);     
  // difference between allocated block size and the required block size
  size_t extraSize = blockSize-reqSize;
  
  // decide if it is a split or a non-split case
  if (extraSize >= MIN_BLOCK_SIZE) { // split case
	//set the used tag
	ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag;
	ptrFreeBlock->sizeAndTags |= TAG_USED;
	
	// pointer to the split-off free block    
	BlockInfo* nextFreeBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
	// set tag preceding as true since the other half of the block was allocated
	nextFreeBlock->sizeAndTags = extraSize | TAG_PRECEDING_USED;
	//size_t freeBlockSize = SIZE(nextFreeBlock->sizeAndTags);
	// set tag used as false since the block is going back to the free list
	nextFreeBlock->sizeAndTags &= ~TAG_USED;
	// boundary tag
	*((size_t*)UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize - WORD_SIZE))) = extraSize;	
	//add the free block back to the free list
	insertFreeBlock(nextFreeBlock);

  } else { // non-split case; allocate the whole blockSize
	ptrFreeBlock->sizeAndTags |= TAG_USED;
  ...
  // return a pointer to the payload of the allocated block = beginning of the free block + header
...
/* Free the block referenced by ptr. */
 void mm_free (void *ptr) {
  ...
  // decrement by WORD_SIZE to move the pointer to the header instead of the payload
  blockInfo = (BlockInfo*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  // size of the payload
  payloadSize = (SIZE(blockInfo->sizeAndTags))- WORD_SIZE;

  // set the TAG_USED to false since it is a free block
  blockInfo->sizeAndTags &= ~TAG_USED;
  
  // next block after the used block
  followingBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptr, SIZE(blockInfo->sizeAndTags));
 
  // change the folloing block's TAG_PRECEDING_USED to false
  followingBlock->sizeAndTags &= ~TAG_PRECEDING_USED;
  
  // insert the block back into a free list
  insertFreeBlock(blockInfo);
  // coalesce with the other blocks on the free list
